...
class Env:
    def __init__(self,n, m, threshold, nround = 15, bounty = 100):
    ...
    def step(self,n):
        ...
        if x < 0 or y < 0 or x >= self.row_size or y >= self.col_size:
            ...
            return np.concatenate((self.grid,self.tiles)), reward, done
        
        if (self.grid[x][y] != 1):
            reward = self.tiles[x][y]*0.1
        else:
            reward = -0.1
...
def main():            
    running_reward = 10
    plot_data = np.array(range(300000))
        ...
        for t in range(1, DEPTH): 
            ...
            if(i_episode % 50000 == 0):
                ...
                if i_episode % 300000 == 0:
                    np.savetxt("plot_data_policynetwork.txt",plot_data)
                    env.render()
